...
ManeuverPIDController::configure(const boost::property_tree::ptree& config)
{
    pidCascade_ = std::make_shared<ManeuverCascade>(&sensorData_, velocityInertial_,
                  accelerationInertial_, &controllerTarget_, &controllerOutput_, &overrideTarget_);
...
ManeuverPIDController::run(RunStage stage)
{
    ...
    {
        auto ipc = ipc_.get();
        overrideTargetSubscription_ = ipc->subscribeOnSharedMemory<OverrideTarget>(
                                          "override_target",
                                          std::bind(&ManeuverPIDController::onManeuverTarget, this, std::placeholders::_1));

        if (!overrideTargetSubscription_.connected())
        {
            APLOG_ERROR << "PIDController: maneuver target subscription failed";
            return true;
        }

        overrideActivationSubscription_ = ipc->subscribeOnSharedMemory<OverrideActivation>(
                                              "override_activation",
                                              std::bind(&ManeuverPIDController::onManeuverActivation, this,
                                                      std::placeholders::_1));

        if (!overrideActivationSubscription_.connected())
        {
...
void
ManeuverPIDController::notifyAggregationOnUpdate(Aggregator& agg)
{
...
ManeuverPIDController::calculateControl()
{
    ...
    // APLOG_ERROR << accelerationInertial_;
...
void
ManeuverPIDController::onManeuverTarget(const OverrideTarget& manTarget)
{
    LockGuard lock(controllerTargetMutex_);
    overrideTarget_ = manTarget;
}

void
ManeuverPIDController::onManeuverActivation(const OverrideActivation& manActivation)
{
    LockGuard lock(controllerTargetMutex_);
    pidCascade_->setManeuverOverride(manActivation);
}

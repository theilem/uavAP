...
ManeuverRatePIDController::run(RunStage stage)
{
	...
	case RunStage::INIT:
	{
		...
		{
			APLOG_ERROR << "ManeuverRatePIDController: Missing Dependencies";
			return true;
		}
	...
	case RunStage::NORMAL:
	{
		...
		overrideSubscription_ = ipc->subscribeOnPackets("override",
				std::bind(&ManeuverRatePIDController::onOverridePacket, this,
						std::placeholders::_1));
		...
		if (auto dh = get<DataHandling>())
		{
			...
			dh->subscribeOnCommand<PIDTuning>(Content::TUNE_PID,
					std::bind(&ManeuverRatePIDController::tunePID, this, std::placeholders::_1));
...
ManeuverRatePIDController::setControllerTarget(const ControllerTarget& target)
{
	...
	if (!io)
	{
		APLOG_ERROR << "Cannot calculate control, IO missing";
		return;
	}
	target_ = target;

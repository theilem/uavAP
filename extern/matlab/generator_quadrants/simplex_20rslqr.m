function [P, Ks, Ad, Bd, u_1, phi_1] = simplex_20rslqr(u_1, v_1, w_1, phi_1, theta_1, p_1, q_1, r_1, Du, Dh, Dphi, pitch_ctrl, throttle_ctrl, aileron_ctrl, yaw_ctrl)
...
%% System Matrix
% As = [Along, Along_corr, zeros(4,2), long0; Alat_corr, Alat, zeros(4,2), lat0; [Hlong, Hlat; Psi_long, Psi_lat], [0,0;0,0], [H0; Psi_0]; zeros(1, 10), 1];
% Bs = [Blong, Blong_corr; Blat_corr, Blat; zeros(3, 4)];
As = [zeros(3), [1, zeros(1, 8); zeros(1,7), 1, 0; zeros(1,8), 1]; zeros(4, 3), Along, Along_corr, zeros(4, 1); zeros(4, 3), Alat_corr, Alat, zeros(4,1); zeros(1,3), Hlong, Hlat, 0];
Bs = [zeros(3, 4); Blong, Blong_corr; Blat_corr, Blat; zeros(1, 4)];
...
uConstraint1 = [-1/Du; zeros(11,1)];
uConstraint2 = [1/Du; zeros(11, 1)];
phiConstraint1 = [0; -1/Dphi; zeros(10, 1)];
phiConstraint2 = [0; 1/Dphi; zeros(10, 1)];
hConstraint1 = [zeros(2, 1); -1/Dh; zeros(9, 1)];
hConstraint2 = [zeros(2, 1); 1/Dh; zeros(9, 1)];
...
%% Time delayed system
continuous = ss(As,Bs,eye(12), zeros(12,4));

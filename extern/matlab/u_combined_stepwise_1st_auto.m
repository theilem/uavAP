...
g = 9.80665;
noiseMag = 0.01;
controlIdx = 3; % Control Type. 0: Throttle, 1: Elevator, 2: Aileron, 3: Rudder
...
for idx = 1:length(datapaths)
    ...
    u_dot = data.u_dot + (g * cos(lin.theta)) * theta;
    x = [u, w, data.q, v, data.p, r, ctrl];
    [rows, cols] = size(x);
    x = x + wgn(rows, cols, mag2db(noiseMag));
    [y,p,crb,s2,xm,pindx, print_info] = auto_swr(x, u_dot, 1, 1, 0.3);

...
g = 9.80665;
noiseMag = 0.01;
controlIdx = 2; % Control Type. 0: Throttle, 1: Elevator, 2: Aileron, 3: Rudder
...
for idx = 1:length(datapaths)
    ...
    w_dot = data.w_dot + (g * sin(lin.theta) * cos(lin.phi)) * theta + (g * cos(lin.theta) * sin(lin.phi)) * phi;
    x = [u, w, data.q, v, data.p, r, ctrl];
    [rows, cols] = size(x);
    x = x + wgn(rows, cols, mag2db(noiseMag));
    
    [y,p,crb,s2,xm,pindx, print_info] = auto_swr(x, w_dot, 1, 1, 0.3);
